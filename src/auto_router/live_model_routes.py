...
def register_live_model_routes(app: FastAPI, state: Any) -> None:
    ...
    async def admin_live_models() -> dict[str, Any]:
        return {"providers": state.live_models.snapshot()}
    ...
    async def refresh_live_models(provider: str | None = None) -> dict[str, Any]:
        ...
        for item in providers:
            records.append((await state.live_models.refresh_provider(item, fetch_provider_models)).to_dict())
